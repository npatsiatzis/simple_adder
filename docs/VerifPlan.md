...
1. **Scope**
   ...
   This document establishes the verification plan for the adder design specified in
...
verification approaches for the adder design as specified in the requirement 
specification number adder_req_001(Specification.md).
...
1. **Non-government Documents**
   ...
   Document #: adder_req_001(Specification.md), Requirement Specification for UART.
...
to mimic using the adder in a system environment. The environment will perform the
following transactions at pseudo-random intervals: 
   1. Create addition requests 
   3. Force resets
   ...
   | Reset | Directed Test | set reset signal active (high) | All components of adder enter reset state |
   | Addition Operation | Constrained Random | Coverage Driven (full cross-coverge for add operands) |    *assert* observed data = expected data, for each test |
